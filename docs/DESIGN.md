...
堆内部使用 `Vec<HeapObject>` 存储对象，`Gc<T>` 为直接索引。GC 回收时**不删除**对象，而是将死对象的槽位标记为"空闲"，放入自由链表（`free_slots: Vec<usize>`）。下次分配时优先复用空闲槽位，池满才追加。

这一设计的核心收益是 **索引稳定性**：无论 GC 运行多少次，已分配的 `Gc<T>` 句柄始终有效，无需回溯更新所有引用。代价是堆只增长不收缩——但 Phase 2 阶段（Snapshot、Yield、Resume）对内存持续增长并不敏感，真正的内存回收可在后续版本引入 compaction 或分代策略。

GC 的标记阶段复用根扫描机制（遍历所有 `CallFrame.locals` 和操作数栈），这也是 Phase 2.3/2.4 snapshot 序列化的同一套基础设施。
...
- 简单的标记-清除 GC（为 snapshot 的堆遍历做准备）
- 堆遍历与序列化（复用 GC 的根扫描）
- 栈帧序列化
- Snapshot 格式定义与读写
