...
mod tests {
    ...
    fn mnemonic_names() {
        ...
        assert_eq!(
            OpCode::TcpConnect { addr: Value::Null }.mnemonic(),
            "tcp_connect"
        );
        ...
        assert_eq!(
            OpCode::TimerSleep { ms: Value::Null }.mnemonic(),
            "timer_sleep"
        );
    }
    ...
    fn display_with_args() {
        ...
        assert_eq!(
            format!("{}", OpCode::FileWrite(HandleId(4))),
            "file_write #4"
        );
        assert_eq!(
            format!(
                ...
                OpCode::FileSeek {
                    handle: HandleId(5),
                    offset: Value::Int(100)
                }
            ),
            "file_seek #5 100"
        );
        assert_eq!(
            format!("{}", OpCode::FileClose(HandleId(6))),
            "file_close #6"
        );
        assert_eq!(
            format!("{}", OpCode::TcpConnect { addr: Value::Null }),
            "tcp_connect null"
        );
        assert_eq!(
            format!("{}", OpCode::HttpGet { url: Value::Null }),
            "http_get null"
        );
        assert_eq!(
            format!(
